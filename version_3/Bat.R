...
# Consultar: .bat_hyperparams$global / .bat_hyperparams$runs[["nombre"]]
# -----------------------------
.bat_hyperparams <- list(
  global = list(
    algorithm          = "BAT",
...
# OPT: recibe dist pre-calculada, tabulate para penalty
evaluate_solution <- function(cluster_assignment, d_cosine, target_cardinality, penalty_weight = 10) {
...
run_bat_algorithm <- function(X, y, target_cardinality,
  ...
  best_score <- evaluate_solution(best_solution, d_cosine, target_cardinality)
  best_seed <- bats[[1]]$seed
  
  for (iteration in seq_len(max_iterations)) {
    for (i in seq_len(n_bats)) {
      bat <- bats[[i]]
      ...
      new_score <- evaluate_solution(new_position, d_cosine, target_cardinality)
...
run_clustering_row <- function(dataset, target_cardinality, dataset_name) {
  ...
  .bat_hyperparams$runs[[dataset_name]] <<- list(
    k                = length(target_cardinality),
    n                = length(y_predict),
